...
In addition to the :ref:`repositories` setup, the package setup is
...
image that are not available in the package manager's native format. {kiwi}
supports the inclusion of regular tar archives via the `archive` element, whose
`name` attribute specifies the filename of the archive ({kiwi} looks for the
...
{kiwi} supports two different methods for removing packages from the
appliance:
...
2. Packages present as a child element of `<packages type="delete">` are removed
   ...
   dependencies and compromising the underlying package database.
