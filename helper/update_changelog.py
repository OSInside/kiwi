...
def main(
    since: Annotated[
        Optional[Path], typer.Option(
            help='changes since the latest entry in the reference file'
        )
    ] = None,
    file: Annotated[
        Optional[Path], typer.Option(
            help='changes from the given file'
        )
    ] = None,
    utc: Annotated[
        Optional[bool], typer.Option(
            '--utc',
            help='print date/time in UTC'
        )
    ] = False,
    fix: Annotated[
        Optional[bool], typer.Option(
            '--fix',
            help='lookup .fix files and apply them'
        )
    ] = False
):
    if not since and not file:
        print('Either --since or --file must be specified')
        sys.exit(1)
    if since and file:
        print('Only one of --since or --file must be specified')
        sys.exit(1)
