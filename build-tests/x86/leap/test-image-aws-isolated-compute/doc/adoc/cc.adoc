...
= Confidential Computing on AWS: Nitro Attestation and Enclaves
Marcus Schäfer
:docinfo:
...
from privileged insiders, compromised hypervisors, or advanced malware
capable of reading system memory. Confidential computing addressed this
attack vector by placing workloads inside a hardware-protected Trusted
Execution Environment (TEE). The memory part of the TEE is encrypted
and strictly access controled ensuring that neither cloud providers,
...
processes, the operating system, and the hypervisor.
...
=== Isolation on AWS
Isolation is achieved using images that provide an overlay-based
read-only system. This system is verified at boot time by `dm-verity`, which
checks all rootfs blocks. In production, instances from these images are
...
=== Memory Encryption and Nitro Enclaves
...
the workload must run as a **Nitro Enclave**.
...
the example enclave see: link:https://build.opensuse.org/package/show/Virtualization:Appliances:Images:Testing_x86:leap/test-image-nitro-enclave[test-image-nitro-enclaves].
